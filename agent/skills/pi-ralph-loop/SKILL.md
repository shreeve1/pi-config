...
**If tmux monitoring:**
```bash
tmux new-session -d -s pi-ralph-loop '.loop/run.sh 2>&1 | tee -a .loop/logs/latest.log'
tmux split-window -h -t pi-ralph-loop 'watch -n 2 .loop/monitor.sh'
tmux attach -t pi-ralph-loop
```
