 ...
 *   catppuccin-mocha · cyberpunk · dracula · ember · everforest · gruvbox
...
export function applyExtensionTheme(
  ...
  // If there are multiple extensions stacked, they each fire session_start
  // and try to apply their own mapped theme. The LAST one to fire wins.
  // We want to skip theme application for all secondary extensions if they are
  // stacked, so the primary extension (first in the array) dictates the theme.
...
// No-op default export — required so Pi doesn't error when auto-scanning the
// extensions directory. themeMap.ts is a shared utility, not a standalone extension.
